...
Func Increment()
	Local $msg = "Введите номер задачи для переключения на нее." & @CRLF & "==Ввести новую задачу." & @CRLF & "-{номер задачи} удалить задачу." & @CRLF & @CRLF & "Текущие задачи: "

	; докидываем в сообщение задачи с номерами
	$msg = $msg & @CRLF & TasksToString()

	Local $result = InputBox($AppName, $msg, Default, Default, 350, 330)
